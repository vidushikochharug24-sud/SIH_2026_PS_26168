...
def latlon_to_enu(lat, lon, ref_lat, ref_lon):
    ...
    relative to a reference fix (ref_lat, ref_lon), making the initial point (0, 0).
    """
...
def generate_synthetic_telemetry(n_steps=1200, dt=0.1):
    """
    Fallback generator if CSV files are missing.
    """
...
def load_data(trip_name="VTB01"):
    ...
    possible_imu_paths = [
        ...
        f"backend/data/S-Vtb{tag[-1]}.csv"
    ]
